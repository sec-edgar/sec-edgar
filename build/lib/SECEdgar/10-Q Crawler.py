# Script to crawl and save 10-Q filing from the Edgar and save that in structured directory format
...
# Procedure to retrieve the CIK codes with the entered companyCode Code
companyCode = input("Enter the companyCode Code : ")
...
while(len(tddoc)!=0):
	#print type(companyCode)
	base_url = "http://www.companyCode.gov/cgi-bin/browse-edgar?action=getcompany&SIC="+str(companyCode)+"&owner=include&match=&start="+str(count)+"&count=40hidefilings=0"
...
# `companyList` now contains the list of companies corresponding to the entered companyCode code.
cikList = []
...
for j in range(len(cikList)):
	if not os.path.exists("Crawled Data/"+str(companyCode)+"/"+str(cikList[j])):
		os.makedirs("Crawled Data/"+str(companyCode)+"/"+str(cikList[j]))
	if not os.path.exists("Crawled Data/"+str(companyCode)+"/"+str(cikList[j])+"/"+str("10-Q")):
		os.makedirs("Crawled Data/"+str(companyCode)+"/"+str(cikList[j])+"/"+str("10-Q"))

for i in range(len(cikList)):
	base_url = "http://www.companyCode.gov/cgi-bin/browse-edgar?action=getcompany&CIK="+str(cikList[i])+"&type=10-Q&dateb=&owner=exclude&output=xml&count=100"	
	#print base_url
	r = requests.get(base_url)
	data = r.text
	...
	for j in range(len(docList)):
		...
		r = requests.get(base_url)
		data = r.text
		path = "Crawled Data/"+str(companyCode)+"/"+str(cikList[i])+"/"+str("10-Q")+"/"+str(docNameList[j])
