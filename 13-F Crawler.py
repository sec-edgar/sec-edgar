...
for j in range(len(cikList)):
	...
	if not os.path.exists("Crawled Data/"+str(sec)+"/"+str(cikList[j])+"/"+str("13-F")):
		os.makedirs("Crawled Data/"+str(sec)+"/"+str(cikList[j])+"/"+str("13-F"))

for i in range(len(cikList)):
	base_url = "http://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK="+str(cikList[i])+"&type=13F&dateb=&owner=exclude&output=xml&count=100"	
	#print base_url
	r = requests.get(base_url)
	data = r.text
	...
	for j in range(len(docList)):
		...
		r = requests.get(base_url)
		data = r.text
		path = "Crawled Data/"+str(sec)+"/"+str(cikList[i])+"/"+str("13-F")+"/"+str(docNameList[j])
