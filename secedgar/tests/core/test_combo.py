...
class TestComboFilings:
    ...
    def test_client_passed_to_objects(self, mock_user_agent):
        ...
        combo = ComboFilings(
            start_date=date(2020, 1, 1), end_date=date(2020, 12, 31), client=client
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                date(2020, 4, 1),
                [(2020, 1, lambda _: True)],
                ["2019-12-28", "2019-12-29", "2019-12-30", "2019-12-31", "2020-04-01"],
            ),
            (
                date(2020, 3, 30),
                date(2020, 10, 2),
    ...
    def test_combo_daily_quarterly_mixed(
        self, start_date, end_date, quarterly_expected, daily_expected, mock_user_agent
    ):
        combo = ComboFilings(
            start_date=start_date, end_date=end_date, user_agent=mock_user_agent
        )
    ...
    @pytest.mark.parametrize("bad_entry_filter", [None, [], (), 0, ""])
    ...
    @pytest.mark.parametrize(
        "good_entry_filter",
        [lambda x: True, lambda x: False, lambda f: f.form_type.lower() == "10-k"],
    )
    ...
    def test_combo_filings_quarterly_lambda(self, mock_user_agent):
        ...
        assert date_list == [
            date(2022, 4, d) for d in range(1, 16)
        ], """
    ...
    def test_combo_no_user_agent_raises_error(self):
        with pytest.raises(TypeError):
            _ = ComboFilings(start_date=date(2022, 2, 28), end_date=date(2022, 4, 15))
