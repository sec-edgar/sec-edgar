...
class TestFilings:
    ...
    def test_client_passed_to_objects(self, mock_user_agent):
        ...
        company = filings(
            cik_lookup="aapl", filing_type=FilingType.FILING_10Q, client=client
        )
        daily = filings(
            start_date=date(2021, 1, 1), end_date=date(2021, 1, 1), client=client
        )
        quarterly = filings(
            start_date=date(2020, 1, 1), end_date=date(2020, 3, 31), client=client
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                dict(count=10, client=NetworkClient(user_agent="Test")),
                NotImplementedError,
            ),
