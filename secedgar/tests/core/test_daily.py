...
class TestDaily:
    ...
    def test_idx_filename(self, date, expected_filename, mock_user_agent):
        assert (
            DailyFilings(date=date, user_agent=mock_user_agent).idx_filename
            == expected_filename
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                cik,
                "http://www.sec.gov/Archives/edgar/data/{cik}/{f}".format(cik=cik, f=f),
            )
    ...
    def test_get_listings_directory(
        self, mock_user_agent, mock_daily_quarter_directory
    ):
        daily_filings = DailyFilings(date(2018, 12, 31), user_agent=mock_user_agent)
    ...
    def test_path_property(self, year, month, day, quarter, mock_user_agent):
        ...
        assert (
            daily_filing.path
            == "Archives/edgar/daily-index/{year}/QTR{quarter}/".format(
    ...
    @pytest.mark.parametrize(
        "date",
        [date(2020, 1, 1), date(2020, 3, 30), date(1993, 12, 31), datetime(2020, 1, 1)],
    )
    ...
    def test_save_with_single_level_date_dir_pattern(
        ...
        daily_filing = DailyFilings(date(2018, 12, 31), user_agent=mock_user_agent)
        daily_filing.save(
            tmp_data_directory, dir_pattern="{date}", date_format="%Y-%m-%d"
        )
    ...
    @pytest.mark.parametrize("cik,file", cik_file_pairs)
    def test_save_with_multi_level_dir_pattern(
        self,
        tmp_data_directory,
        mock_user_agent,
        mock_daily_quarter_directory,
        ...
        daily_filing = DailyFilings(date(2018, 12, 31), user_agent=mock_user_agent)
        daily_filing.save(
            tmp_data_directory, dir_pattern="{date}/{cik}", date_format="%Y-%m-%d"
        )
    ...
    def test_save_with_multi_level_dir_pattern_date_not_first(
        ...
        daily_filing = DailyFilings(date(2018, 12, 31), user_agent=mock_user_agent)
        daily_filing.save(
            tmp_data_directory, dir_pattern="{cik}/{date}", date_format="%Y-%m-%d"
        )
    ...
    def test__get_tar_bad_year_quarter(self, year, quarter, mock_user_agent):
        with pytest.raises(ValueError):
            d = DailyFilings(
                date=date(year, utils.get_month(quarter), 1), user_agent=mock_user_agent
            )
            d._get_tar_urls()
    ...
    @pytest.mark.parametrize("bad_entry_filter", ["entry_filter", True, False, 1, 1.0])
    ...
    @pytest.mark.parametrize(
        "good_entry_filter",
        [lambda _: True, lambda _: False, lambda f: f.name.lower().startswith("a")],
    )
    ...
    def test_user_agent_client_none(self):
        with pytest.raises(TypeError):
            _ = DailyFilings(date=date(2020, 1, 1), user_agent=None, client=None)
