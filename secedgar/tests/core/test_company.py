...
def mock_cik_validator_get_single_cik(monkeypatch):
    ...
    monkeypatch.setattr(
        CIKLookup, "get_ciks", lambda *args, **kwargs: {"aapl": "0000320193"}
    )
...
class TestCompanyFilings:
    ...
    bad_dates = [1, 2020010101, "2020010101", "2020", "0102"]
    ...
    def test_count_returns_exact(
        self, mock_user_agent, mock_cik_validator_get_single_cik, mock_single_cik_filing
    ):
        count = 10
        aapl = CompanyFilings(
            user_agent=mock_user_agent,
    ...
    @pytest.mark.parametrize("bad_filing_type", ("10-k", "10k", "10-q", "10q", 123))
    ...
    @pytest.mark.parametrize("bad_filing_type", ("10-j", "10-k", "ssd", "invalid", 1))
    ...
    @pytest.mark.parametrize("no_urls", ({}, {"aapl": [], "fb": [], "msft": []}))
    def test_save_no_filings_raises_error(
        self, tmp_data_directory, monkeypatch, mock_user_agent, no_urls
    ):
    ...
    def test_filing_save_multiple_ciks(
        self,
        tmp_data_directory,
        mock_user_agent,
    ...
    @pytest.mark.smoke
    def test_filing_save_multiple_ciks_smoke(
        self, tmp_data_directory, real_test_client
    ):
        f = CompanyFilings(
            ["aapl", "amzn", "msft"],
            FilingType.FILING_10Q,
    ...
    def test_filing_save_single_cik(
        self,
        tmp_data_directory,
        mock_user_agent,
        ...
        f = CompanyFilings(
            "aapl", FilingType.FILING_10Q, user_agent=mock_user_agent, count=3
        )
        f.save(tmp_data_directory)
        assert len(os.listdir(tmp_data_directory)) > 0

    @pytest.mark.smoke
    def test_filing_save_single_cik_smoke(self, tmp_data_directory, real_test_client):
        f = CompanyFilings(
            "aapl", FilingType.FILING_10Q, client=real_test_client, count=3
        )
        f.save(tmp_data_directory)
        assert len(os.listdir(tmp_data_directory)) > 0
    ...
    @pytest.mark.parametrize(
        "count,raises_error",
        [(5, False), (10, False), (20, True), (30, True), (40, True)],
    )
    @pytest.mark.filterwarnings("ignore::DeprecationWarning")
    # For collections.abc warning 3.8+
    def test_filings_warning_lt_count(
        self,
        recwarn,
        count,
        raises_error,
    ...
    @pytest.mark.smoke
    @pytest.mark.slow
    def test_filing_simple_example_smoke(self, tmp_data_directory, mock_user_agent):
        ...
        assert (
            len(os.listdir(tmp_data_directory)) > 0
        ), "No file or directory created after save."
    ...
    @pytest.mark.parametrize(
        "bad_ownership",
        ["notright", "_exclude", "_include", "notvalid", 1, True, False],
    )
