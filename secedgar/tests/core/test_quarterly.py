...
def mock_master_idx_file(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda *args: MockResponse(
            datapath_args=["filings", "master", "master.idx"]
        ).text,
    )
...
class TestQuarterly:
    ...
    def test_bad_quarter(self, mock_user_agent, bad_quarter, expected_error):
        with pytest.raises(expected_error):
            _ = QuarterlyFilings(
                year=2020, quarter=bad_quarter, user_agent=mock_user_agent
            )

    def test_good_quarters(self, mock_user_agent):
        for quarter in range(1, 5):
            mf = QuarterlyFilings(
                year=2019, quarter=quarter, user_agent=mock_user_agent
            )
    ...
    def test_always_false_entry_filter(self, mock_user_agent, mock_master_idx_file):
        quarterly_filing = QuarterlyFilings(
            year=1993,
            quarter=4,
    ...
    def test_save(
        ...
        mock_user_agent,
        mock_filing_data,
        mock_quarterly_quarter_directory,
        ...
        quarterly_filing = QuarterlyFilings(
            year=1993, quarter=4, user_agent=mock_user_agent
        )
    ...
    def test_clean_path(self, mock_user_agent, original_path, clean_path):
        quarterly_filing = QuarterlyFilings(
            year=2000, quarter=1, user_agent=mock_user_agent
        )
