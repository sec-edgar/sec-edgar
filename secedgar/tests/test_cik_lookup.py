...
def mock_get_cik_map(monkeypatch):
    ...
    monkeypatch.setattr(
        requests, "get", MockResponse(content=bytes(response_json, "utf-8"))
    )
...
class TestCIKLookup(object):
    ...
    @pytest.mark.parametrize("lookup,expected", ciklookup_results)
    def test_cik_lookup_returns_correct_values(
        self, mock_client_cik_lookup, lookup, expected, mock_get_cik_map
    ):
    ...
    @pytest.mark.smoke
    @pytest.mark.parametrize("lookup,expected", ciklookup_results)
    def test_cik_lookup_returns_correct_values_smoke(
        self, lookup, expected, real_test_client
    ):
        look = CIKLookup(lookup, client=real_test_client)
        assert look.lookup_dict == expected

    def test_cik_lookup_lookups_property(self, mock_client_cik_lookup):
        multiple_company_lookup = CIKLookup(
            ["aapl", "msft", "fb"], client=mock_client_cik_lookup
        )
        assert multiple_company_lookup.lookups == ["aapl", "msft", "fb"]

    def test_multiple_results_company_name_search(
        self, mock_client_cik_lookup, mock_single_cik_multiple_results_response
    ):
    ...
    def test_validate_cik_after_cik_lookup(
        self, mock_client_cik_lookup, mock_single_cik_not_found
    ):
    ...
    @pytest.mark.parametrize("bad_lookups", [[], "", ["AAPL", 4, "FB"]])
    ...
    @pytest.mark.parametrize("bad_lookup", ["", 4])
    ...
    def test_params_reset_after_get_cik(
        self, ticker_lookups, mock_client_cik_lookup, mock_single_cik_lookup_outside_map
    ):
        lookup = CIKLookup(lookups=ticker_lookups, client=mock_client_cik_lookup)
        lookup._get_cik_from_html(ticker_lookups[0])
        assert lookup.params.get("CIK") is None and lookup.params.get("company") is None

    @pytest.mark.parametrize("lookup,cik", cikmap_ticker_results)
    ...
    @pytest.mark.parametrize("lookup,cik", cikmap_company_results)
