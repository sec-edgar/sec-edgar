...
from secedgar.tests.core.test_daily import mock_daily_quarter_directory  # noqa: F401
...
class TestCLI:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ValueError,
            ),  # no filings available if 0 picked
        ],
    )
    def test_cli_filing_bad_inputs(
        self, user_input, expected_exception, tmp_data_directory
    ):
        check_bad_inputs(
            cli,
            user_input=user_input,
    ...
    def test_cli_filing_multiple_companies_input_smoke(
        self, user_input, count, tmp_data_directory
    ):
        if count is not None:
            user_input = user_input.format(count)
        result = run_cli_command(cli, user_input, tmp_data_directory)
    ...
    @pytest.mark.parametrize(
        "user_input,expected_exception",
        [("daily", SystemExit), ("daily -d 2020", ValueError)],
    )
    def test_cli_daily_bad_inputs(
        self, user_input, expected_exception, tmp_data_directory
    ):
        check_bad_inputs(
            cli,
    ...
    @pytest.mark.parametrize(
        "user_input",
        ["daily -d 20201113", "filing -l aapl -l msft -l amzn -t FILING_10Q -n 10"],
    )
    def test_cli_requires_user_agent(self, user_input, tmp_data_directory):
        check_bad_inputs(
            cli, user_input, SystemExit, tmp_data_directory, user_agent=None
        )
