...
class TestFiling(object):
    def test_count_returns_exact(self,
        ...
        count = 10
        aapl = CompanyFilings(cik_lookup="aapl", filing_type=FilingType.FILING_10Q, count=count)
    ...
    def test_count_setter_on_init(self, count):
        filing = CompanyFilings(cik_lookup="aapl", filing_type=FilingType.FILING_10Q, count=count)
    ...
    def test_end_date_setter(self, date, expected):
        f = CompanyFilings("aapl", FilingType.FILING_10Q, start_date=datetime.datetime(
    ...
    def test_txt_urls(self, mock_cik_validator_get_single_cik, mock_single_cik_filing):
        aapl = CompanyFilings(cik_lookup="aapl", filing_type=FilingType.FILING_10Q, count=10)
    ...
    def test_validate_cik_type_inside_filing(self, bad_cik_lookup):
        with pytest.raises(TypeError):
            CompanyFilings(cik_lookup=bad_cik_lookup, filing_type=FilingType.FILING_10K)
    ...
    def test_filing_save_multiple_ciks(self, tmp_data_directory,
        ...
        f = CompanyFilings(["aapl", "amzn", "msft"], FilingType.FILING_10Q, count=3)
        f.save(tmp_data_directory)
    ...
    def test_filing_simple_example(self, tmp_data_directory):
        my_filings = CompanyFilings(cik_lookup="IBM", filing_type=FilingType.FILING_10Q)
