...
class TestNetworkClient:
    ...
    def test_client_good_response_single_filing_type_passes(
        self, mock_single_filing_type_good_response, client
    ):
        assert client.get_response("path")

    def test_client_good_response_multiple_cik_results_passes(
        self, mock_multiple_cik_results_good_response, client  # noqa: E501
    ):
        assert client.get_response("path")

    def test_client_good_response_single_filing_passes(
        self, mock_single_filing_page_good_response, client
    ):
        assert client.get_response("path")
    ...
    def test_bad_backoff_factor_setter(self, bad_backoff_factor, mock_user_agent):
        with pytest.raises(TypeError):
            _ = NetworkClient(
                user_agent=mock_user_agent, backoff_factor=bad_backoff_factor
            )

    @pytest.mark.parametrize("good_backoff_factor", [-1, -1.0, 1, 1.0, 2, 10])
    ...
    @pytest.mark.parametrize("good_rate_limit", range(1, 11))
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("rate_limit", range(1, 10))
    def test_rate_limit_requests_per_second(
        self, mock_user_agent, tmp_data_directory, rate_limit, mock_filing_response
    ):
        client = NetworkClient(user_agent=mock_user_agent, rate_limit=rate_limit)
        min_seconds = 3
        num_requests = rate_limit * min_seconds
        inputs = [
            ("https://google.com", os.path.join(tmp_data_directory, str(i)))
