...
if [[ $(uname) == "Linux" ]]; then
    ...
    sudo apt-get update && sudo apt-get install python-dev libxml2-dev libxslt-dev libz-dev python3-async-timeout
fi
...
if [[ "$1" -eq "docs" ]]; then
    pip install ipython sphinx_rtd_theme sphinx sphinx-autobuild
fi
